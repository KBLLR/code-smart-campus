...
const sensorDashboardContainer = document.getElementById("sensor-dashboard"); // Ref to container
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  try {
    ...
    try {
      console.log("[Toolbar Fallback] Instantiating toolbar…");
      new Toolbar({
        onToggleGroup: (group, visible) => {
          // replicate your original toggle logic here:
          Object.values(labelManager.getLabels()).forEach((label) => {
            if (
              label.userData?.registry?.type === group &&
        ...
        initialLabels: labelManager.getLabels(),
        setupInstance: setup,
      });
      console.log("✅ Toolbar instantiated via DOMContentLoaded fallback.");
    } catch (error) {
      console.error("❌ Toolbar fallback failed:", error);
    }
  } catch (error) {
...
whenReady("labels", (labelsData) => {
  ...
  try {
    // ** CRITICAL: Ensure .toolbar exists in index.html **
    new Toolbar({
      onToggleGroup: (group, visible) => {
        /* ... */
      },
    ...
    console.error("❌ Failed Toolbar init:", error);
  }

  // Check for potentially invalid labelsData
  ...
  // Proceed with Toolbar creation inside a try...catch block
  try {
    // Instantiate the Toolbar class
    new Toolbar({
      // Callback function passed to the Toolbar for handling individual data layer toggles
      onToggleGroup: (group, visible) => {
        if (!labelsData) {
          console.warn(
            "[Toolbar Callback] onToggleGroup called, but labelsData is missing.",
          );
          return;
        }
        // Iterate through the labels and set visibility based on type match
      ...
      // Pass the actual label data object to the Toolbar constructor
      initialLabels: labelsData,
      // Pass the fully initialized Setup instance (for camera controls etc.)
      setupInstance: setup,
    });
    ...
    // Catch and log any errors during Toolbar instantiation or setup
...
function loop() {
  ...
  // Use optional chaining in case setup or orbCtrls isn't fully ready yet.
