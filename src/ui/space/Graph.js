...
import { Color } from '@alienkitty/space.js/src/math/Color.js';
import { Vector2 } from '@alienkitty/space.js/src/math/Vector2.js';
// import { SVGPathProperties } from '../path/SVGPathProperties.js'; // Replaced with native SVG
import { Easing } from '@alienkitty/space.js/src/tween/Easing.js';
...
import { clearTween, delayedCall, tween } from '@alienkitty/space.js/src/tween/Tween.js';
...
export class Graph extends Interface {
    constructor({
        ...
        // Helper SVG path for calculations
        this.helperPath = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    ...
    calculateLookup() {
        ...
        this.helperPath.setAttribute('d', this.pathData);
        const length = this.helperPath.getTotalLength(); // properties.getTotalLength();
        const lookup = [];

        let i = 0;

        while (i <= 1) {
            lookup.push(this.helperPath.getPointAtLength(i * length)); // properties.getPointAtLength(i * length)
