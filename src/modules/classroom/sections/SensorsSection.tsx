...
export const SensorsSection: React.FC<SensorsSectionProps> = ({
  ...
  const liveSensors = useSensorStream(classroom.id);
  ...
  const items = [
    {
      ...
      status: s.temperatureC != null
        ? (s.temperatureC < 18 ? 'cold' : s.temperatureC > 24 ? 'hot' : 'good')
        : 'unknown',
    },
    {
      label: 'CO₂',
      value: s.co2ppm != null ? `${s.co2ppm.toFixed(0)} ppm` : '—',
      status: s.co2ppm != null
        ? (s.co2ppm > 1200 ? 'bad' : s.co2ppm > 800 ? 'warning' : 'good')
        : 'unknown',
    },
    {
      label: 'Occupancy',
    ...
    {
      label: 'Noise',
      value: s.noiseDb != null ? `${s.noiseDb.toFixed(0)} dB` : '—',
      status: s.noiseDb != null
        ? (s.noiseDb > 70 ? 'bad' : s.noiseDb > 55 ? 'warning' : 'good')
        : 'unknown',
    },
    {
      label: 'Light',
      value: s.lightLux != null ? `${s.lightLux.toFixed(0)} lux` : '—',
      status: s.lightLux != null
        ? (s.lightLux < 200 ? 'dim' : s.lightLux > 800 ? 'bright' : 'good')
        : 'unknown',
    },
    {
      label: 'Humidity',
      value: s.humidityPercent != null ? `${s.humidityPercent.toFixed(0)}%` : '—',
      status: s.humidityPercent != null
        ? (s.humidityPercent < 30 || s.humidityPercent > 60 ? 'warning' : 'good')
        : 'unknown',
    },
  ];
